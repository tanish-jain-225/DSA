...
public:
    ...
    bool
    isSubtree(TreeNode* s,
              TreeNode* t) // TC: O(N*M) where N is the number of nodes in tree
                           // s and M is the number of nodes in tree t)
    {
        if (s == NULL || t == NULL) // Base case: All in one
        {
            return s == t; // Both should be null to be the same
        }

        if (s->val == t->val &&
            isSameTree(s, t)) // Check if the current nodes match and if the
                              // trees are the same
        {
            return true; // Found a match, check if the trees are the same
        }

        return isSubtree(s->left, t) ||
               isSubtree(s->right,
                         t); // Recursively check left and right subtrees
    }
};